...
export const formatters = {
  currency: (amount: number | string, options?: Intl.NumberFormatOptions): string => {
  ...
  compactCurrency: (amount: number | string): string => {
    const numAmount = typeof amount === 'string' ? parseFloat(amount) : amount;
  ...
  // Date formatting
  date: (date: string | Date, format = DATE_FORMATS.SHORT): string => {
    try {
  ...
  creditScore: (score: number, grade?: string): string => {
    const formattedScore = score.toFixed(1);
    return grade ? `${formattedScore} (${grade})` : formattedScore;
  },

  // Risk level formatting with color coding
  riskLevel: (riskLevel: string): { text: string; color: string } => {
  ...
  grade: (grade: string): { text: string; color: string; description: string } => {
    const gradeMap: Record<string, { text: string; color: string; description: string }> = {
  ...
  businessType: (type: string): string => {
  ...
  arrayToString: (arr: string[], conjunction = 'and'): string => {
  ...
  scoreBand: (score: number): { band: string; color: string } => {
    if (score >= 80) return { band: 'Excellent', color: '#10B981' };
    if (score >= 65) return { band: 'Good', color: '#3B82F6' };
    if (score >= 50) return { band: 'Fair', color: '#F59E0B' };
    if (score >= 35) return { band: 'Poor', color: '#F97316' };
  ...
  loanTenure: (months: number): string => {
    if (months < 12) {
  ...
  // Ratio formatting
  ratio: (numerator: number, denominator: number, suffix = '%'): string => {
  ...
  creditHistory: (repaidPercentage: number, status: string): string => {
  ...
  psychometricScore: (score: number): { interpretation: string; color: string } => {
    if (score >= 90) return { interpretation: 'Excellent financial behavior', color: '#10B981' };
    if (score >= 80) return { interpretation: 'Good financial behavior', color: '#3B82F6' };
    if (score >= 60) return { interpretation: 'Average financial behavior', color: '#F59E0B' };
    if (score >= 40) return { interpretation: 'Below average financial behavior', color: '#F97316' };
    return { interpretation: 'Poor financial behavior', color: '#EF4444' };
  },

  // Red flag severity formatting
  redFlagSeverity: (severity: string): { text: string; color: string; icon: string } => {
    const severityMap: Record<string, { text: string; color: string; icon: string }> = {
