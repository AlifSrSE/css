...
import { ReportType, ReportFormat, GenerateReportRequest } from '../types/reports';
...
export const useReports = () => {
  ...
  // Report actions
  const setCurrentReportData = useCallback((report: any) => {
  ...
  // Report operations
  const generateNewReport = useCallback(async (request: GenerateReportRequest) => {
  ...
  const downloadReportFile = useCallback(async (reportId: string, format?: ReportFormat) => {
  ...
  const deleteReportFile = useCallback(async (reportId: string) => {
  ...
  // Bulk operations
  const bulkDownloadReports = useCallback(async (reportIds: string[], format: ReportFormat = 'pdf') => {
    if (!hasPermission('export_data')) {
      throw new Error('Insufficient permissions to download reports');
    }
  ...
  const isReportDownloadable = useCallback((reportId: string) => {
    const report = getReportById(reportId);
  ...
  const getUniqueReportTypes = useMemo(() => {
    const types = new Set(generatedReports.map(report => (report as any).report_type));
  ...
  const filterByType = useCallback((reportType: ReportType) => {
    updateFilters({ report_type: reportType });
  }, [updateFilters]);

  const filterByStatus = useCallback((status: 'pending' | 'generating' | 'completed' | 'failed') => {
    updateFilters({ status });
  }, [updateFilters]);

  const filterByDateRange = useCallback((dateRange: DateRange) => {
    updateFilters({ date_range: dateRange });
  }, [updateFilters]);

  // Report template helpers
  const getAvailableTemplates = useCallback((reportType?: ReportType) => {
    if (!reportType) return templates;
